...
namespace ECJ.Tests
{
    public abstract class ECJTestBase : AbpIntegratedTestBase<ECJTestModule>
    {
        ...
        protected void UsingDbContext(Action<ECJDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<ECJDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<ECJDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<ECJDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<ECJDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ECJDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<ECJDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<ECJDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<ECJDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<ECJDbContext, Task<T>> func)
        {
            T result;
